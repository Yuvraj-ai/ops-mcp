...
- [x] **Fix 2 — make rejection-path audit/idempotency write failures observable, not silently swallowed.** **DONE (2026-08-05.)** **The premise needed correcting first:** the failures were *not* silently swallowed — all three best-effort catch blocks in `src/db/queries.ts` (`logAction`, `getIdempotencyResult`, `storeIdempotencyResult`) already called `console.error` with the underlying error. Verified by test before changing anything. What was genuinely missing is the *context* the client's wording specifies ("tool name, order ID, and the error"): the old lines read `Failed to store idempotency result: error: duplicate key…` with no indication of which tool, order, or key, and no statement of consequence — effectively undiagnosable in Render's log stream. Implemented: each line now carries a greppable tag, the identifying fields, and what the failure *means* operationally. Real output from a test run:
  ...
  Tags are `[audit-write-failed]`, `[idempotency-check-failed]`, `[idempotency-store-failed]`. Non-blocking behavior is unchanged and now explicitly asserted. 8 new checks (48/48 main suite) cover: the write does not throw, the failure is logged, and the line names tool / order / key / underlying error. A metrics counter remains a nice-to-have, not implemented.
...
- **[2026-08-05] Fix 2 — rejection-path log observability.** Enriched the three best-effort catch blocks in `src/db/queries.ts` with structured, greppable context. **Correction to the client's finding:** these failures were never silently swallowed — `console.error` was already being called in all three. The real defect was that the lines were unidentifiable: no tool, no order, no key, no statement of consequence, so a failure in Render's log stream could not be traced to the call that caused it. Now tagged `[audit-write-failed]` / `[idempotency-check-failed]` / `[idempotency-store-failed]`, each naming the tool plus order or key, and stating what the failure means operationally (e.g. "key was NOT stored, so a retry with this key will re-execute"). Non-blocking behavior unchanged and now asserted rather than assumed. **8 new checks; main suite 48/48, concurrency suite 18/18, build clean.**
- **[2026-08-05] Fix 1c + test infrastructure migration to real Postgres.**
