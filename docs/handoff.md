...
- [x] **Fix the oversell race condition in `reconfirm_order`** — DONE: stock decrement is now atomic conditional `UPDATE ... WHERE sku = $2 AND available_qty >= $1`; if 0 rows affected, throws and rolls back. Verified against live Supabase: A1024 (0 stock) correctly rejected, stock not decremented. 3 new test cases pass (20/20 total).
