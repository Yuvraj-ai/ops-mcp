...
src/db/queries.ts        — OpsRepository data access class (transactions, audit, idempotency)
src/tools/definitions.ts — all 7 MCP tool definitions
src/server.ts            — Express + MCP SDK wiring, stateless HTTP transport
src/tests/tools.test.ts  — 30 runtime checks (all passing, pg-mem)
...
- [x] **Fix oversell race condition in `reconfirm_order`** — DONE (see changelog). 3 new test cases pass (24/24 total).
- [x] **Make audit log + idempotency keys transactional** — DONE: success-path audit + idempotency INSERTs now run inside `reconfirmOrder()`/`issueRefund()` transactions (same `BEGIN/COMMIT/ROLLBACK` as the mutation). Handlers only write audit + idempotency standalone for replay + error/rejection paths (when the repo transaction has already rolled back). Verified against live Supabase: 11/11 live checks pass. 30/30 tests pass.
...
- [2026-08-01] — Made audit log + idempotency keys transactional. Success-path audit (`action_log INSERT`) and idempotency key storage (`idempotency_keys INSERT`) now execute inside `reconfirmOrder()`/`issueRefund()` transactions — same `BEGIN/COMMIT/ROLLBACK` as the mutation itself, so they're committed atomically. On error/ROLLBACK, the handler still persists audit + idempotency records as standalone inserts (best-effort, captures the rejection). Handlers refactored: success path delegates entirely to repo (repo handles in-txn audit + idempotency); replay and error/rejection paths write audit + idempotency outside the rolled-back transaction. Added design spec `docs/superpowers/specs/2026-08-01-transactional-idempotency-design.md`. 30/30 tests pass. Verified against live Supabase: 11/11 live checks pass (refund+audit+idempotency atomicity, replay, oversell prevention, reconfirm happy path), demo state restored.